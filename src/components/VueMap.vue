...
watch(props.coordinates), (oldCoords, newCoords) => {drawMap(newCoords) {
  L.map("mapContainer").flyTo(newCoordinates, 5); // check flyTo function instead of setView
...
onMounted(() => {
  ...
  if (!props.coordinates) {
    map = L.map("mapContainer").setView(props.coordinates, 1); // check flyTo function instead of setView
  } else {
